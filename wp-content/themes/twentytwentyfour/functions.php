...
function custom_user_page_selection() {
	if (is_user_logged_in()) {
			...
			$args = array(
					...
					's' => $search_query, // Используем параметр 's' для поиска
			);

			$pages = get_posts($args);
			$output = '<form method="get" action="">';
			$output .= '<input type="text" name="search" value="' . esc_attr($search_query) . '" placeholder="Search pages...">';
			...
			foreach ($pages as $page) {
					...
					$output .= '</div>';
					$output .= '</div>';
					$output .= '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" class="methodix-menu-card-icon" viewBox="0 0 24 24" fill="none">
					...
					$output .='<div class="methodix-display-flex-column methodix-mb-1">';
					...
					$output .= '<div class="methodix-suggestion-chip">
